...
The application instructs the AI to return structured JSON data. The following schemas represent the expected structure (note: actual AI responses may occasionally vary slightly).

### Identification Schema (`IdentifySpeciesResult`)
...
### Diagnosis Schema (`DiagnoseHealthResult`)
...
```json
{
  ...
  "preventionTips": ["Improve air circulation", "Avoid overhead watering"],
  "urgentActions": [],
  "recoveryTimeWeeks": 3,
