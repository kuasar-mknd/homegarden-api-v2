...
## 2025-05-23 - DoS via Unbounded Streams & Hanging Requests
**Vulnerability:** External API calls lacked timeouts (potential resource exhaustion) and image downloads lacked size limits (potential OOM).
**Learning:** Fetch calls to external services must always have timeouts and response body reading must be size-bounded.
**Prevention:** Use `AbortSignal.timeout()` and custom stream readers for large downloads.
