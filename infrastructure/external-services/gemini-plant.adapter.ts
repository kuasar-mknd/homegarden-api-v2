...
export class GeminiPlantAdapter implements AIIdentificationPort, AIDiagnosisPort {
  // Optimization: Lazy instantiate AI SDK and models to reduce startup overhead
  ...
  private getIdentificationModel(): GenerativeModel {
  ...
  private getDiagnosisModel(): GenerativeModel {
  ...
  async identifySpecies(request: IdentifySpeciesRequest): Promise<IdentifySpeciesResult> {
    ...
    try {
      ...
      // Call Gemini
      const result = await this.getIdentificationModel().generateContent([prompt, imagePart])
  ...
  async diagnoseHealth(request: DiagnoseHealthRequest): Promise<DiagnoseHealthResult> {
    ...
    try {
      ...
      // Call Gemini
      const apiResult = await this.getDiagnosisModel().generateContent([prompt, imagePart])
