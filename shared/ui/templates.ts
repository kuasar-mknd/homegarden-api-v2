...
const ARROW_LEFT_ICON = `<svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 12H5"/><path d="M12 19l-7-7 7-7"/></svg>`
...
export function getNotFoundPageHtml(path: string): string {
  ...
  return baseLayout({
    ...
    <main id="main">
      ...
      <div class="btn-group">
        <button onclick="if(history.length>1){history.back()}else{location.href='/'}" class="btn btn-secondary">${ARROW_LEFT_ICON}Go Back</button>
