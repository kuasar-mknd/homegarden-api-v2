...
export function getNotFoundPageHtml(path: string): string {
  ...
  return baseLayout({
    ...
    <main id="main">
      ...
      <div class="btn-group">
        <button onclick="if(history.length>1){history.back()}else{window.location.href='/'}" class="btn btn-secondary" aria-label="Go back to previous page">${BACK_ICON}Go Back</button>
