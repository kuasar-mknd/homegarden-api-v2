...
// Optimization: "Static Response Caching" - Pre-compute template and split for O(N) concatenation
...
const NOT_FOUND_PAGE_PARTS = NOT_FOUND_PAGE_TEMPLATE.split('{{PATH}}')

export function getNotFoundPageHtml(path: string): string {
  const safePath = escapeHtml(path)
  return NOT_FOUND_PAGE_PARTS[0] + safePath + NOT_FOUND_PAGE_PARTS[1]
}
